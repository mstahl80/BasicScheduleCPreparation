...
class CloudKitManager: ObservableObject {
    ...
    private func setupSubscriptions() {
        ...
        let permissionSubscription = CKQuerySubscription(
            id: permissionSubscriptionID,
            recordType: userPermissionRecordType,
    ...
    func fetchUserIdentity(completion: @escaping (CKUserIdentity?, Error?) -> Void) {
        container.fetchUserRecordID { (recordID: CKRecord.ID?, error: Error?) in
            ...
            guard let recordID = recordID else {
                completion(nil, NSError(domain: "CloudKitManager", code: 1, userInfo: [NSLocalizedDescriptionKey: "No user record ID"]))
                return
            }
            
            self.container.discoverUserIdentity(withUserRecordID: recordID) { (identity: CKUserIdentity?, error: Error?) in
    ...
    func createInvitation(email: String, completion: @escaping (String?, Error?) -> Void) {
        isLoading = true
        ...
        fetchUserIdentity { (identity: CKUserIdentity?, error: Error?) in
            var creatorName = "Unknown User"
            if let identity = identity {
                if let nameComponents = identity.nameComponents {
                    creatorName = PersonNameComponentsFormatter().string(from: nameComponents)
                } else if let userRecordID = identity.userRecordID {
                    creatorName = userRecordID.recordName
                }
            }
    ...
    func fetchInvitations() {
        ...
        if #available(iOS 15.0, macOS 12.0, *) {
            ...
            #if compiler(>=5.5.2) && canImport(Darwin)
            if #available(iOS 15.0, macOS 12.0, *) {
                // Use queryResultBlock on iOS 15+
                ...
                // Fall back to queryCompletionBlock for older iOS
        ...
        if #available(iOS 15.0, macOS 12.0, *) {
            ...
            privateDatabase.fetch(withQuery: query, inZoneWith: nil, desiredKeys: nil, resultsLimit: 1) { [weak self] result in
                ...
                case .success(let (matchResults, _)):
                    ...
                    self.fetchUserIdentity { (identity: CKUserIdentity?, error: Error?) in
                        var userName = "Unknown User"
                        if let identity = identity {
                            if let nameComponents = identity.nameComponents {
                                userName = PersonNameComponentsFormatter().string(from: nameComponents)
                            } else if let userRecordID = identity.userRecordID {
                                userName = userRecordID.recordName
                            }
                        }
                        ...
                        self.privateDatabase.save(record) { (savedRecord: CKRecord?, error: Error?) in
                            ...
                            self.container.fetchUserRecordID { (recordID: CKRecord.ID?, error: Error?) in
                                ...
                                permissionRecord["role"] = "editor"  // Default role for invited users
                            ...
                            self.privateDatabase.fetch(withQuery: query, inZoneWith: nil, desiredKeys: nil, resultsLimit: 1) { result in
                                    switch result {
                                        ...
                                        private func setupLegacyCloudKitSharing(completion: @escaping (CKShare?, Error?) -> Void) {
                                            ...
                                            record["creator"] = UserAuthManager.shared.getCurrentUserDisplayName() as CKRecordValue
